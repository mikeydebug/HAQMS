...
import { 
  ...
  ArrowRight, ShieldAlert, CheckCircle, Volume2, HeartPulse, TestTube2
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  // FRONTEND PERFORMANCE FIX: Debounce the API fetch to prevent spamming backend and heavy list re-renders on every keystroke
  useEffect(() => {
  ...
  const handleBookAppointment = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setBookingReason('');
        if (user.role === 'DOCTOR') // eslint-disable-next-line
    fetchDoctorWorklist();
      } else {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <main className="ml-64 pt-16 p-6 sm:p-8 relative z-10 max-w-7xl mx-auto">
        
        

        {/* Global Notifications Panel */}
        {checkinMessage && (
          <div className="p-4 mb-6 rounded-xl bg-white/10 border border-white/20 text-white flex items-center justify-between text-sm">
        ...
        <AnimatePresence mode="wait">
          <PageTransition key={activeTab} transitionKey={activeTab}>
            {/* ==============================================================
          ...
          <div className="space-y-8">
            <div className="grid gap-8 lg:grid-cols-3">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="minimal-card p-6 rounded-2xl shadow-md border border-white/10">
                  ...
                  <div className="flex gap-4 mb-6">
                    <div className="relative flex-1 rounded-lg shadow-sm">
                      ...
                      <input
                        ...
                        className="block w-full pl-9 pr-3 py-2 border border-white/10 bg-black text-white focus:border-white/30 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent text-sm"
                      />
                    </div>
                    ...
                    <select
                      ...
                      className="px-3 py-2 border border-white/10 bg-black text-white focus:border-white/30 rounded-lg text-white text-sm focus:outline-none"
                    >
                  ...
                  {/* Table listing */}
                  {patientsLoading ? (
                    <p className="text-center py-6 text-zinc-500 animate-pulse text-sm">Synchronizing table data...</p>
                  ) : patients.length === 0 ? (
                    <p className="text-center py-6 text-zinc-500 text-sm">No registered patients match this filter.</p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800 text-sm text-left">
                        <thead>
                          <tr className="text-zinc-500 uppercase tracking-widest text-xxs font-bold border-b border-white/10">
                          ...
                          {patients.map((p) => (
                            <tr key={p.id} className="hover:bg-white/5 transition-colors">
                              <td className="py-3.5 font-bold text-white">
                                {p.name}
                                {p.email && <span className="block text-xxs text-zinc-500 font-normal mt-0.5">{p.email}</span>}
                              </td>
                              <td className="py-3.5 text-zinc-400 font-medium">{p.phoneNumber}</td>
                              ...
                              <td className="py-3.5 text-right space-x-2">
                                <button
                                  ...
                                  className="text-xxs px-2.5 py-1 rounded bg-white/10 text-white font-bold hover:bg-white hover:text-white transition-colors"
                                >
                                  Check In
                                </button>
                  ...
                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-white/10">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-3 py-1 rounded border border-slate-200 dark:border-slate-700 hover:bg-white/10 disabled:opacity-50 text-xs font-semibold"
                      >
                        Prev
                      </button>
                      <button
              ...
              {/* Registration Form */}
              <div className="minimal-card p-6 rounded-2xl shadow-md border border-white/10 h-fit">
                <h3 className="text-lg font-extrabold text-white flex items-center gap-2 mb-4">
                ...
                {regMessage && (
                  <div className={`p-3 text-sm rounded-lg mb-4 ${regMessage.startsWith('Success') ? 'bg-white/10 text-white border border-white/20' : 'bg-rose-500/15 text-rose-500 border border-rose-500/20'}`}>
                    {regMessage}
                  </div>
                )}

                <form onSubmit={handleRegisterPatient} className="space-y-4 text-xs font-semibold text-zinc-300">
                  <div>
                    ...
                    <input
                      ...
                      className="block w-full px-3 py-2 border border-white/10 bg-black text-white focus:border-white/30 rounded-lg text-white text-sm focus:outline-none"
                    />
                  </div>
                  ...
                  <button
                    type="submit"
                    className="transition-colors w-full py-2.5 bg-indigo-500 text-white hover:bg-indigo-600 font-extrabold text-sm rounded-lg shadow-md transition-colors duration-300 mt-2"
                  >
        ...
        {activeTab === 'book' && (
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            <div className="minimal-card p-6 rounded-2xl shadow-md border border-white/10">
              ...
              {bookingMessage && (
                <div className={`p-3 text-sm rounded-lg mb-4 ${bookingMessage.startsWith('Success') ? 'bg-white/10 text-white border border-white/20' : 'bg-rose-500/15 text-rose-500 border border-rose-500/20'}`}>
                  {bookingMessage}
                </div>
              )}

              <form onSubmit={handleBookAppointment} className="space-y-4 text-xs font-semibold text-zinc-300">
                <div>
                  ...
                  <span className="text-xxs text-zinc-500 block mt-1">If client is missing, register them in the Directory tab first.</span>
                </div>
        ...
        {activeTab === 'appointments' && (
          <div className="space-y-6">
            <div className="minimal-card p-6 rounded-2xl border border-white/10 shadow-md">
              <h3 className="text-lg font-extrabold text-white flex items-center gap-2 mb-4">
                <CalendarDays className="h-5 w-5 text-white" />
              ...
              {doctorAppointments.length === 0 ? (
                <p className="text-center py-6 text-zinc-500 text-sm">No appointments scheduled for you today.</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800 text-sm text-left">
                    <thead>
                        ...
                        <tr key={app.id} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="py-3.5">
                            <button
                              ...
                              className="font-bold text-white hover:underline hover:text-teal-700 transition-colors"
                            >
                              {app.patient ? app.patient.name : 'Unknown Patient'}
                            </button>
                            <span className="block text-xxs text-zinc-500 mt-0.5">Age: {app.patient?.age}</span>
                          </td>
                          <td className="py-3.5 text-zinc-400 font-semibold">{app.reason || 'None provided'}</td>
                          <td className="py-3.5">
                            <span className={`inline-flex px-2 py-0.5 rounded text-xxs font-extrabold tracking-wide uppercase ${app.status === 'COMPLETED' ? 'bg-white/10 text-white' : app.status === 'CANCELLED' ? 'bg-rose-500/10 text-rose-500' : 'bg-amber-500/10 text-amber-500'}`}>
                              {app.status}
                            </span>
                          </td>
                          <td className="py-3.5 text-right space-x-2">
                            {app.status === 'PENDING' && (
                              <>
                                <button
                                  ...
                                  className="text-xxs px-2.5 py-1 rounded bg-white/10 text-white font-extrabold hover:bg-white hover:text-white transition-colors"
                                >
                                  Check In Patient
                                </button>
                                <button
                                  onClick={() => handleCompleteAppointment(app.id)}
                                  className="text-xxs px-2.5 py-1 rounded bg-slate-100 dark:bg-slate-800 text-zinc-300 font-extrabold hover:bg-white hover:text-white transition-colors"
                                >
                                  Complete
                                </button>
                              </>
            ...
            {selectedPatientHistory && (
              <div className="minimal-card p-6 rounded-2xl border border-white/10 shadow-md space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-lg font-extrabold text-white">
                      Medical Records: {selectedPatientHistory.name}
                    </h3>
                    <p className="text-xxs font-bold text-zinc-500 uppercase tracking-widest mt-1">
                      Gender: {selectedPatientHistory.gender} | Contact: {selectedPatientHistory.phoneNumber}
                    </p>
                  </div>
                  <button 
                    onClick={() => setSelectedPatientHistory(null)}
                    className="text-xs font-bold text-zinc-500 hover:text-slate-600"
                  >
                    Close
                  </button>
                </div>
                ...
                <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-xs space-y-2">
                  <h4 className="font-bold text-zinc-500 uppercase tracking-wider">Clinical Background Information</h4>
                  
                  {/* FRONTEND CRASH FIX: Use safe accessor or ternary operator for nullable properties */}
                  <p className="text-zinc-300 leading-5 text-sm font-semibold">
                    {selectedPatientHistory.medicalHistory ? selectedPatientHistory.medicalHistory.toUpperCase() : 'None provided'}
                  </p>
                </div>
                ...
                <div className="pt-2 flex justify-between items-center text-xs">
                  ...
                  <Link 
                    ...
                    className="text-white font-extrabold hover:underline flex items-center gap-1"
                  >
        ...
        {activeTab === 'queue' && (
          <div className="minimal-card p-6 rounded-2xl border border-white/10 shadow-md">
            ...
            {doctorQueue.length === 0 ? (
              <p className="text-center py-6 text-zinc-500 text-sm">No checked-in patients in queue today.</p>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {doctorQueue.map((t) => (
                  <div
                    key={t.id}
                    className={`p-5 rounded-2xl border shadow-md relative overflow-hidden flex flex-col justify-between ${t.status === 'CALLING' ? 'border-white bg-white/10' : 'border-white/10 bg-white/5'}`}
                  >
                    <div className="flex justify-between items-start">
                      <span className="text-2xl font-black text-white">Token #{t.tokenNumber}</span>
                      <span className={`px-2 py-0.5 rounded text-xxs font-extrabold tracking-wide uppercase ${t.status === 'CALLING' ? 'bg-white text-black' : t.status === 'COMPLETED' ? 'bg-white/10 text-white' : 'bg-amber-500/10 text-amber-500'}`}>
                        {t.status}
                      </span>
                    </div>

                    <div className="mt-4">
                      <h4 className="text-xs font-bold text-white">{t.patient.name}</h4>
                      <p className="text-xxs text-zinc-500 mt-0.5">Contact: {t.patient.phoneNumber}</p>
                    </div>
        ...
        {activeTab === 'reports' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <StatCard title="Blood Pressure" value="110/70" icon={HeartPulse} trend="up" trendValue="10%" iconBgColor="bg-rose-500/20 text-rose-500" />
              <StatCard title="Pulse Rate" value="72 bpm" icon={Activity} trend="down" trendValue="02%" iconBgColor="bg-emerald-500/20 text-emerald-500" />
              <StatCard title="Glucose Level" value="95 mg/dL" icon={Activity} trend="up" trendValue="05%" iconBgColor="bg-amber-500/20 text-amber-500" />
              <StatCard title="Blood Count" value="9,456/mL" icon={TestTube2} trend="down" trendValue="22%" iconBgColor="bg-indigo-500/20 text-indigo-500" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              <ActivityChart />
              <div className="cliniva-card p-6 rounded-2xl border border-[var(--border)] shadow-md">
                ...
                <button
                  ...
                  className="transition-colors px-4 py-2 bg-teal-600 text-white font-extrabold text-xs rounded-lg shadow hover:bg-teal-700 disabled:opacity-50 transition-colors"
                >
              ...
              {adminReportLoading ? (
                ...
                <div className="p-8 text-center bg-white/5 rounded-xl text-zinc-500 text-xs font-semibold border border-dashed border-slate-200 dark:border-slate-700">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-3 p-3 bg-amber-500/10 text-zinc-300 text-xs rounded-lg border border-amber-500/20 leading-5">
                  ...
                  <div className="grid gap-4 sm:grid-cols-3">
                    <div className="p-4 bg-white/5 border border-white/10 rounded-xl">
                      <span className="text-xxs uppercase tracking-wider text-zinc-500 font-bold">Total Physicians</span>
                      <h4 className="text-2xl font-black text-white mt-1">{adminReportData.data.length}</h4>
                    </div>
                    <div className="p-4 bg-white/5 border border-white/10 rounded-xl">
                      <span className="text-xxs uppercase tracking-wider text-zinc-500 font-bold">Sum appointments</span>
                      <h4 className="text-2xl font-black text-white mt-1">
                        {adminReportData.data.reduce((sum, item) => sum + item.totalAppointments, 0)}
                      </h4>
                    </div>
                    <div className="p-4 bg-white/5 border border-white/10 rounded-xl">
                      <span className="text-xxs uppercase tracking-wider text-zinc-500 font-bold">Total Sales ($)</span>
                      <h4 className="text-2xl font-black text-white mt-1">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {adminReportData.data.map((item) => (
                          <tr key={item.id} className="hover:bg-white/5 transition-colors">
                            <td className="py-3.5 font-bold text-white">
                              {item.name}
                              <span className="block text-xxs text-white font-semibold uppercase mt-0.5">{item.specialization}</span>
                            </td>
                            <td className="py-3.5 text-zinc-400">{item.department}</td>
                            <td className="py-3.5 text-center text-zinc-400">
                              {item.completedAppointments} Completed / {item.totalAppointments} Total
                            </td>
                            <td className="py-3.5 text-center font-bold text-white">{item.todayQueueSize} in queue</td>
                            <td className="py-3.5 text-right font-bold text-white">${item.revenue}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        {activeTab === 'physicians' && (
          <div className="minimal-card p-6 rounded-2xl border border-white/10 shadow-md space-y-6">
            <div>
              <h3 className="text-lg font-extrabold text-white flex items-center gap-2">
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className="transition-colors px-5 py-2 bg-indigo-500 text-white hover:bg-indigo-600 font-bold text-xs rounded-lg hover:bg-slate-800 dark:hover:bg-teal-400 transition-colors"
              >
                Execute SQL Query
              </button>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {doctorsList.map((doc) => (
                <div
                  key={doc.id}
                  className="p-5 rounded-2xl border border-white/10 bg-white/5 flex flex-col justify-between"
                >
                  <div>
                    <span className="inline-flex px-2 py-0.5 rounded text-xxs font-extrabold tracking-wide uppercase bg-white/10 text-white mb-2">
                      {doc.department}
                    </span>
                    <h4 className="font-extrabold text-white">{doc.name}</h4>
                    <p className="text-xs text-zinc-500 mt-0.5">{doc.specialization}</p>
                  </div>
                  <div className="mt-6 pt-3 border-t border-white/10/80 flex justify-between items-center text-xs font-semibold text-slate-500">
                    <span>Exp: {doc.experience} yrs</span>
                    <span className="font-bold text-white">Fee: ${doc.consultationFee}</span>
                  </div>
                </div>
              ))}
            </div>

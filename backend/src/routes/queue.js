...
router.post('/checkin', authenticate, async (req, res) => {
  try {
    ...
    // PERFORMANCE/CONCURRENCY FIX: Use a transaction with a row lock to ensure atomicity
    const newToken = await prisma.$transaction(async (tx) => {
      // Lock the Doctor row to prevent concurrent check-ins from generating the same token number
